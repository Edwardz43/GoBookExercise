...
func getComic(n int) (Comic, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return comic, fmt.Errorf("cant't get comic %d %s", n, resp.Status)
	}
...
func getComics() (chan Comic, error) {
	...
	comicNumbers := make(chan int, 1*nworkers)
...
func indexComics(comics chan Comic) (WordIndex, NumIndex) {
	...
	for comic := range comics {
		...
		for scanner.Scan() {
			...
			wordIndex[token][comic.Number] = true
		}
	}
...
func fetcher(comicNumbers chan int, comics chan Comic, done chan int) {
	for n := range comicNumbers {
